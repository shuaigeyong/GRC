...
<html>
    ...
    <body>
                    ...
                    <div class="box-8">
                        <div class="content-box">
                            <div class="container">
                                <!-- Member List -->
                                <div class="heading">
                                    <p class="add-title">Member List</p><br>
                                </div>
        ...
        <?php
        $i = 1;
        $sql = $con->prepare("SELECT * FROM users WHERE user_type = 'user' AND account_available = 1"); 
        $sql->execute();
        ...
        if($result && $result->num_rows > 0){
            while($row = $result->fetch_array()){
                ...
                printf("<tr>
                            ...
                            <td>%s</td>
                            <td>
                                <button class='btn btn-outline-danger' data-id='{$id}' > <a class='text-light'>Banned</a></button>
                            </td> 
                        </tr>", $i++, $id, $name, $email, ($verify_status == 1)? "Verified" : "Not verified", $join_date, $id);
            }
        }
        else{
            printf("<tr><td colspan='7' style='text-align: center;'>No member has registered yet!</td></tr>");
        }
      ?>
                </table>
              </div>
    ...
    <!-- Delete Button -->
    <script>
        // The confirmation message for the delete account
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                swalWithBootstrapButtons.fire({
                  title: 'Are you sure?',
                  text: "This account will be permanently banned until further notice!",
                  icon: 'warning',
                  ...
                  if (result.isConfirmed) {
                    ...
                    setTimeout(function(){
                        window.location.href = "deletemember.php?deleteid=" + id;
                    }, 2000); // 延迟才2秒执行delete
                  } else if (
